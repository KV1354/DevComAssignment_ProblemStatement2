...
class SyncService:
    ...
    def onMessage(self, data: dict):
        ...
        if data == {} or data==None:
          return
        if data['type']=='record':
          self.serverRecords.append(data)
          return
        elif data['type']=='probe':
          toBeSent=self.serverRecords[int(data['from']):]
          return {'type': 'update', 'from' : int(data['from']), 'data' : toBeSent}
